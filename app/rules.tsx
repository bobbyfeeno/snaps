...
const GAMES: GameRule[] = [
  ...
  },
  {
    emoji: '🎲',
    name: 'Aces & Deuces',
    rules: `Every hole has an Ace (best score) and a Deuce (worst score) — and everyone is in the action.\n\nStandard payout:\n• The Ace collects the set amount from EVERY other player\n• The Deuce pays the set amount to EVERY other player\n• Middle scores pay the Ace and collect from the Deuce — netting to $0 when a single bet amount is used\n\nConvention: many groups set the Ace bet at 2× the Deuce bet, so middle players actually lose a small amount each hole.\n\nTie rules (app variant): tied-best players all count as Aces; tied-worst all count as Deuces.\n\nNote: The most common standard rule is that ties cancel — if two players tie for best, there's no Ace that hole (push). Agree on your group's tie rule before the round.\n\nIf everyone ties, it's always a push. Stacks well alongside any other game.`,
  },
  {
    emoji: '⚖️',
    name: 'Quota',
